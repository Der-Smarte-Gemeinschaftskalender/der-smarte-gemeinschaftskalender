...
import { apolloClient, getFetchPolicy } from '@/lib/mobilizonApi';
...
export const findNextEvents = async (
    limit: number = 5,
    beginsOn: Date | null = null,
    endsOn: Date | null = null,
    searchTarget: string = 'INTERNAL'
): Promise<{
    __typename: string;
    elements: IEvent[];
    total: number;
...
export const searchEvents = async (
    ...
    term = '',
    searchTarget: string | null = null,
    beginsOn: string = new Date().toISOString(),
    endsOn: string | null = null,
    categoryOneOf: string[] | null = null,
    statusOneOf: string[] | null = null,
    languageOneOf: string[] | null = null,
    ...
    organizedEvents(limit: 12, afterDatetime: $organizedEventsAfterDateTime, order: BEGINS_ON) {
      total
        ...
        `,
            variables: {
                preferredUsername,
                organizedEventsAfterDateTime: organizedEvents.afterDateTime
                    ? organizedEvents.afterDateTime
...
export const findOrganisationOptions = async (valueUsername = false, limit: number = 1000) => {
    ...
    return [{ value: 'ALL', text: 'Alle Organisationen' }, ...groupOptions];
};
